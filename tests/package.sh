...
pkg=$(find . -maxdepth 1 -name 'omarchy-netflix-*.pkg.tar.zst' -print -quit)
[[ -n $pkg ]]
...
desktop-file-validate /usr/share/applications/omarchy-netflix.desktop
grep -Fx 'Name=Netflix' /usr/share/applications/omarchy-netflix.desktop
[[ $(pacman -Qoq /usr/bin/omarchy-netflix) == omarchy-netflix ]]
[[ $(omarchy-netflix --version) == 'omarchy-netflix 0.1.0' ]]
...
grep -Fx -- 'https://www.netflix.com/' "$HOME/browser-args"
! grep -q -- '--app=' "$HOME/browser-args"
grep -Fx -- "--user-data-dir=$XDG_CONFIG_HOME/omarchy-netflix/chrome" "$HOME/browser-args"
[[ $(stat -c %a "$XDG_CONFIG_HOME/omarchy-netflix/chrome") == 700 ]]
echo saved > "$XDG_CONFIG_HOME/omarchy-netflix/chrome/login-marker"
...
echo '# custom' >> "$XDG_DATA_HOME/omarchy-netflix/omarchy-netflix"
if omarchy-netflix-migrate-local; then exit 1; fi
[[ -f "$XDG_DATA_HOME/applications/omarchy-netflix.desktop" ]]
bash "$1/install.sh"
omarchy-netflix-migrate-local
omarchy-netflix-migrate-local
[[ ! -e "$XDG_DATA_HOME/applications/omarchy-netflix.desktop" ]]
[[ ! -e "$XDG_DATA_HOME/omarchy-netflix/omarchy-netflix" ]]
[[ $(find "$XDG_DATA_HOME/omarchy-app-migration" -name omarchy-netflix.desktop | wc -l) == 1 ]]
grep -Fx saved "$XDG_CONFIG_HOME/omarchy-netflix/chrome/login-marker"
